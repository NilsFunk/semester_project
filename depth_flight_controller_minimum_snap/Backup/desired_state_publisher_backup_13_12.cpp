...
namespace depth_flight_controller {
    ...
    {
        if (msg.valid == true && ros::Time::now() - most_recent_path_generation_ > ros::Duration(0.3))
        {
            if (msg.side != 0)
            {
                ...
                path_new_ = generatePath(msg.Y, msg.depth, msg.side, state_estimate_);
            ...
            } else
            {
                ...
                path_new_ = generateStraightPath(msg.depth, state_estimate_);
    ...
    std::vector<quad_msgs::QuadDesiredState> DesiredStatePublisher::generatePath(double target_angle_rad, double target_depth, int target_side, QuadState state_estimate)
    {
        ...
        static const float super_factor = 1;
        ...
        {
            quad_msgs::QuadDesiredState desired_state;
            desired_state.position.x = cos(state_yaw)*path_x_pos_smoothed.at(0) - sin(state_yaw)*target_side*(path_y_pos_smoothed.at(0))+ state_x;
            desired_state.position.y = sin(state_yaw)*path_x_pos_smoothed.at(0) + cos(state_yaw)*target_side*(path_y_pos_smoothed.at(0)) + state_y;
            desired_state.position.z = 1.6;

            desired_state.velocity.x = cos(state_yaw)*path_x_vel_smoothed.at(0) - sin(state_yaw)*target_side*(path_y_vel_smoothed.at(0));
            desired_state.velocity.y = sin(state_yaw)*path_x_vel_smoothed.at(0) + cos(state_yaw)*target_side*(path_y_vel_smoothed.at(0));
            desired_state.velocity.z = 0;

            desired_state.acceleration.x = cos(state_yaw)*path_x_acc_smoothed.at(0) - sin(state_yaw)*target_side*(path_y_acc_smoothed.at(0));
            desired_state.acceleration.y = sin(state_yaw)*path_x_acc_smoothed.at(0) + cos(state_yaw)*target_side*(path_y_acc_smoothed.at(0));
